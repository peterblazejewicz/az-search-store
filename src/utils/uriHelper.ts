...
export function searchParameterValidator(
  parameters: Store.SearchParameters,
): void {
  // todo validate params
}

export function suggestParameterValidator(
...
export function buildPostBody(
  parameters: Store.SearchParameters | Store.SuggestionsParameters,
  input: string,
  validator: (
    parameters: Store.SearchParameters | Store.SuggestionsParameters,
  ) => void,
  ...
  Object.keys(parameterMap).forEach(parameter => {
    ...
    !(value == null) && parameter !== 'apiVersion'
...
function getGlobalFilter(globalFilters: { [key: string]: string }): string {
  let filters = Object.keys(globalFilters)
    .filter(key => {
...
export function buildSuggestionsURI(
  config: Store.Config,
  parameters: Store.Parameters,
): string {
  const { service, index } = config;
