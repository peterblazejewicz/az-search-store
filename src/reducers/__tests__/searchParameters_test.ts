...
describe('reducers/searchParameters', () => {
  it('should return initial state when given empty input', () => {
    expect(
      searchParameters.searchParameters(
        <Store.SearchParameters>undefined,
        <searchParametersActions.SearchParametersAction>{},
      ),
  ...
  it('should properly set all searchParameters', () => {
    ...
    expect(
      searchParameters.searchParameters(
        initialState,
        searchParametersActions.setSearchParameters(testParameters),
      ),
  ...
  it('should properly update subset of searchParameters', () => {
    ...
    expect(
      searchParameters.searchParameters(
        initialState,
        searchParametersActions.updateSearchParameters(testParameters),
      ),
    ).toEqual(expectedParams);
  });
  ...
  it('should correctly page to various values, properly observing limits.', () => {
    ...
    expect(
      searchParameters.searchParameters(
        testParameters,
        searchParametersActions.setPage(-4),
      ).skip,
    ).toEqual(0);
    expect(
      searchParameters.searchParameters(
        testParameters,
        searchParametersActions.setPage(1),
      ).skip,
    ).toEqual(0);
    expect(
      searchParameters.searchParameters(
        testParameters,
        searchParametersActions.setPage(10001),
      ).skip,
    ).toEqual(100000);
    expect(
      searchParameters.searchParameters(
        testParameters,
        searchParametersActions.setPage(10002),
      ).skip,
    ).toEqual(100000);
    expect(
      searchParameters.searchParameters(
