...
const testResults = [{ text: 'foo' }, { text: 'bar' }, { text: 'buzz' }];
...
describe('reducers/facets', () => {
  ...
  it('should add a date range facet', () => {
    const expectedFacet: Store.RangeFacet = {
      ...
      filterClause: '',
      facetClause:
        'foo,values:1970-01-01T00:00:01.990Z|1970-01-01T00:00:02.017Z',
    };
    expect(
      reducer(
        facets.initialState,
  ...
  it('should add a numeric checkbox facet', () => {
    ...
    expect(
      reducer(
        facets.initialState,
        facetsAction.addCheckboxFacet(
          expectedFacet.key,
          expectedFacet.dataType,
        ),
      ),
    ).toEqual({
      facets: {
  ...
  it('should set the range for a date range facet', () => {
    ...
    const expectedFacet: Store.RangeFacet = {
      ...
      upperBucketCount: 0,
      filterClause:
        'foo ge 1970-01-01T00:00:01.999Z and foo le 1970-01-01T00:00:02.015Z',
      facetClause:
        'foo,values:1970-01-01T00:00:01.999Z|1970-01-01T00:00:02.015Z',
    };
    const initialFacets: Store.Facets = {
      globalFilters: {},
      facetMode: 'simple',
    ...
    expect(
      reducer(
        initialFacets,
        facetsAction.setFacetRange('foo', new Date(1999), new Date(2015)),
      ),
    ).toEqual(expectedFacets);
  });
  it('should toggle the value of a checkbox facet value, then toggle another value, verify facet and filter clauses', () => {
    ...
    expect(
      reducer(
        initialFacets,
        facetsAction.toggleCheckboxFacetSelection('foo', 'a'),
      ),
    ).toEqual(firstToggleExpectedFacets);
    expect(
      reducer(
        firstToggleExpectedFacets,
        facetsAction.toggleCheckboxFacetSelection('foo', 'b'),
      ),
  ...
  it('should set incoming values for facets, ignoring @odata type annotations', () => {
    ...
    const facetResults: { [key: string]: Store.FacetResult[] } = {
      foo: [{ value: 'c', count: 10 }, { value: 'd', count: 17 }],
      bar: [
