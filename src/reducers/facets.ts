...
export function facets(
  state: Store.Facets = initialState,
  action: FacetsAction,
): Store.Facets {
  switch (action.type) {
    case 'SET_FACET_MODE':
      return setFacetMode(state, action);
    case 'ADD_RANGE_FACET':
      return addRangeFacetAction(state, action);
    ...
    case 'SET_GLOBAL_FILTER':
      return setGlobalFilter(state, action);
    default:
      return state;
  }
}
...
function clearFacetsSelections(
  state: Store.Facets,
  action: ClearFacetsSelectionsAction,
): Store.Facets {
...
function setFacetsValues(
  state: Store.Facets,
  action: SetFacetsValuesAction,
): Store.Facets {
  let facets: { [key: string]: Store.Facet } = {};
  ...
  keysToUpdate.forEach(key => {
    ...
    switch (facet.type) {
      case 'CheckboxFacet':
        facets[key] = setCheckboxFacetValues(
          facet as Store.CheckboxFacet,
          facetResults,
        );
        break;
      case 'RangeFacet':
        facets[key] = setRangeFacetValues(
          facet as Store.RangeFacet,
          facetResults,
        );
        break;
      default:
...
function setRangeFacetValues(
  facet: Store.RangeFacet,
  facetResults: Store.FacetResult[],
): Store.RangeFacet {
...
function setCheckboxFacetValues(
  facet: Store.CheckboxFacet,
  facetResults: Store.FacetResult[],
): Store.CheckboxFacet {
...
function updateFacetsValues(
  state: Store.Facets,
  action: UpdateFacetValuesAction,
): Store.Facets {
  const updatedFacets: { [key: string]: Store.Facet } = {};
  // filter out @odata type annotations
  const keys = Object.keys(action.facets).filter(key => {
    return key.toLowerCase().indexOf(odataString) < 0;
  });
  keys.forEach(key => {
    ...
    switch (facet.type) {
      ...
      case 'CheckboxFacet':
        ...
        const updatedFacet: Store.CheckboxFacet = hasSelection
          ? mergeCheckboxFacetValues(checkboxFacet, currentItem)
          : setCheckboxFacetValues(checkboxFacet, currentItem);
...
function mergeCheckboxFacetValues(
  facet: Store.CheckboxFacet,
  facetResults: Store.FacetResult[],
): Store.CheckboxFacet {
  let values: { [key: string]: Store.CheckboxFacetItem } = {};
  const currentItemKeys = facetResults.map(item => {
  ...
  Object.keys(facet.values).forEach(valueKey => {
    ...
    if (updateIndex >= 0) {
      ...
      values[valueKey] = {
        count: item.count,
        value: item.value,
        selected: facet.values[item.value]
          ? facet.values[item.value].selected
          : false,
      };
    } else {
...
function setFacetMode(
  state: Store.Facets,
  action: SetFacetModeAction,
): Store.Facets {
  const { facetMode } = action;
  return updateObject(state, { facetMode });
}

function getRangeFacetClause(
...
function addRangeFacetAction(
  state: Store.Facets,
  action: AddRangeFacetAction,
): Store.Facets {
  ...
  const rangeFacet: Store.RangeFacet = {
    ...
    upperBucketCount: 0,
    filterClause: '',
    facetClause: getRangeFacetClause(
      dataType,
      key,
      filterLowerBound,
      filterUpperBound,
...
function addCheckboxFacet(
  state: Store.Facets,
  action: AddCheckboxFacetAction,
): Store.Facets {
...
function setFacetRange(
  state: Store.Facets,
  action: SetFacetRangeAction,
): Store.Facets {
  const { key, lowerBound, upperBound } = action;
  const existingFacet = state.facets[key];
  if (existingFacet.type !== 'RangeFacet') {
    throw new Error(
      `SET_FACET_RANGE must be called on facet of type 'RangeFacet', actual: ${existingFacet.type}`,
    );
  }
  const existingRangeFacet = existingFacet as Store.RangeFacet;
  const newRangeFacet = updateObject(existingRangeFacet, {
    filterLowerBound: lowerBound,
    filterUpperBound: upperBound,
  });
  const filter = buildRangeFilter(newRangeFacet);
  const facetClause = getRangeFacetClause(
    newRangeFacet.dataType,
    newRangeFacet.key,
    lowerBound,
    upperBound,
  );
  const newFacetWithFilter = updateObject(newRangeFacet, {
    filterClause: filter,
    facetClause,
  });
  const facets = updateObjectAtKey(state.facets, newFacetWithFilter, key);
  ...
  let clauses = selectedFacets.map(selectedValue => {
    ...
    switch (facet.dataType) {
      ...
      case 'string':
        ...
        break;
      case 'collection':
        clause = `${facet.key}/any(t: t eq '${facet.values[selectedValue]
          .value}')`;
        break;
      default:
        clause = '';
...
function buildRangeFilter(facet: Store.RangeFacet): string {
  ...
  if (
    facet.min === facet.filterLowerBound &&
    facet.max === facet.filterUpperBound
  ) {
    return '';
  }
