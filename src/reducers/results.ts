...
export function results(
  state: Store.SearchResults = initialState,
  ...
  switch (action.type) {
    ...
    case 'RECEIVE_RESULTS':
      let results = state.resultsProcessor
        ? state.resultsProcessor(action.results)
        : action.results;
    ...
    case 'APPEND_RESULTS':
      results = state.resultsProcessor
        ? state.results.concat(state.resultsProcessor(action.results))
