...
describe('actions/facets', () => {
  it('should create action to set facet mode', () => {
    expect(facetsActions.setFacetMode('advanced')).toEqual({
      type: 'SET_FACET_MODE',
      facetMode: 'advanced',
    });
  });
  it('should create action to to add numeric range facet', () => {
    expect(facetsActions.addRangeFacet('foo', 'number', 0, 10)).toEqual({
  ...
  it('should create action to add numeric checkbox facet', () => {
    expect(facetsActions.addCheckboxFacet('bar', 'number')).toEqual({
  ...
  it('should create action to add collection checkbox facet', () => {
    expect(facetsActions.addCheckboxFacet('bar', 'collection')).toEqual({
      type: 'ADD_CHECKBOX_FACET',
      key: 'bar',
      dataType: 'collection',
      count: 5,
  ...
  it('should create action to toggle checkbox facet', () => {
    expect(facetsActions.toggleCheckboxFacetSelection('foo', 'bar')).toEqual({
  ...
  it('should create action to set ranges for facet', () => {
    expect(facetsActions.setFacetRange('foo', 0, 10)).toEqual({
