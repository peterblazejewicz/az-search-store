...
export const setFacetsValues = (facets: {
...
export const updateFacetsValues = (facets: {
  [key: string]: Store.FacetResult[];
}): FacetsAction => ({
  type: 'UPDATE_FACETS_VALUES',
  facets,
...
export const addCheckboxFacet = (
  key: string,
  dataType: Store.CheckboxDataType,
  count: number = 5,
  sort: Store.FacetSortingMode = 'count',
): FacetsAction => ({
  type: 'ADD_CHECKBOX_FACET',
  key,
  dataType,
...
export const toggleCheckboxFacetSelection = (
  key: string,
  value: string | number,
): FacetsAction => ({
  type: 'TOGGLE_CHECKBOX_SELECTION',
...
export const clearFacetsSelections = (): FacetsAction => ({
  type: 'CLEAR_FACETS_SELECTIONS',
});

export const setGlobalFilter = (key: string, filter: string): FacetsAction => ({
  type: 'SET_GLOBAL_FILTER',
  key,
  filter,
});
